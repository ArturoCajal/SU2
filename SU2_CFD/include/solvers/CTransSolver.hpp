...
#include "CSolver.hpp"
#include "../variables/CTransVariable.hpp"
 ...
 * \author A. Cajal.
 */
class CTransSolver : public CSolver {
protected:
  ...
  su2double
  lowerlimit[MAXNVAR] = {0.0},  /*!< \brief contains lower limits for transition variables. */
  upperlimit[MAXNVAR] = {0.0},  /*!< \brief contains upper limits for transition variables. */
  Gamma,                        /*!< \brief Fluid's Gamma constant (ratio of specific heats). */
  Gamma_Minus_One,              /*!< \brief Fluids's Gamma - 1.0  . */
  ***Inlet_TransVars = nullptr;  /*!< \brief Transition variables at inlet profiles */
...
private:
  ...
  /*!
   * \brief Compute the viscous flux for the transitional equation at a particular edge.
...
public:
   ...
   * \param[in] config - Definition of the particular problem.
   */
  CTransSolver(CGeometry* geometry, CConfig *config);
  ...
  */
  ...
  /*!
   * \brief Set custom transition variables at the vertex of an inlet.
  ...
  inline void SetInlet_TransVar(unsigned short val_marker,
    ...
    if (val_marker >= nMarker)
    ...
    else if (Inlet_TransVars == nullptr || Inlet_TransVars[val_marker] == nullptr)
      ...
      SU2_MPI::Error("Out-of-bounds index used for inlet transitio variable.", CURRENT_FUNCTION);
    else
